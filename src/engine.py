...
class Trainer:
    ...
    def __init__(
        ...
        scheduler : Optional[torch.optim.lr_scheduler.LRScheduler], optional
            Планировщик learning rate в эпохах, by default None
    ...
    def validate(self, val_loader: DataLoader) -> None:
        ...
        if val_loss < self.best_val_loss:
            checkpoint_fp = Path(self.writer.get_logdir()) / f"model_{self.global_step}.pth"
            self.save_checkpoint(checkpoint_fp)
            self.best_val_loss = val_loss
            self.best_model_fp = checkpoint_fp
...
def evaluate_loader(loader: DataLoader, model: torch.nn.Module) -> Dict[str, float]:
    with torch.no_grad():
        ...
        return {"acc": np.mean(accs_batches), "f1": f1_score(targets, preds, average="macro")}
